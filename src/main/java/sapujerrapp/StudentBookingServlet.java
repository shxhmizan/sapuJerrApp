...
public class StudentBookingServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			List<BookingEntity> bookings = bookingDao.getUserBookings(user);
		
			System.out.println("Getting bookings for user id: " + user.getUserId());
			/*
			bookings.forEach((BookingEntity booking) ->{
				try {
					response.getWriter().println(booking.getBookingId());
				} catch (IOException e) {
